...
{ // thông số dialog
    ...
    // DIALOG
    // ======
    var dropdown1_array = ["Glass", "Luminous", "Led", "Silicon"];
...
button1.onClick = function () {
    ...
    if (statictext1.text !== "...") {
        ...
        else if (String(dropdown1.selection) == "Silicon") {
    ...
    else { alert("Plese select Json file") }
    ...
    { // tạo nhãn
        ...
        for (var i = 0; i <= arr.length - 1; i++) {
            for (var j = 0; j <= arr[i].length - 1; j++) {
                ...
                var folder1 = Folder("~/Desktop/in an/Glass " + (i + 1) + " ngay " + day + "/tem");
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Glass " + (i + 1) + " ngay " + day + "/tem/" + arr[i][j].stt + ".jpg"), JPEGSaveOptions, true, Extension.LOWERCASE);
...
function createTableLuminous(data) {
    ...
    for (var i = 0; i <= arr.length - 1; i++) { // loop làm file in
        ...
        { // lưu file khung
            ...
            if (!folder1.exists) { folder1.create(); }
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + (i + 1) + " ngay " + day + "/khung.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
        ...
        {// lưu file in
            ...
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + (i + 1) + " ngay " + day + "/in.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
        ...
        }
        {
            ...
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + (i + 1) + " ngay " + day + "/white.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
        }
        app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);
    } // hết làm file
    { // tạo nhãn
        ...
        for (var i = 0; i <= arr.length - 1; i++) {
            for (var j = 0; j <= arr[i].length - 1; j++) {
                moveTem(arr[i][j], day, mounth);
                var folder1 = Folder("~/Desktop/in an/Luminous " + (i + 1) + " ngay " + day + "/tem");
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Luminous " + (i + 1) + " ngay " + day + "/tem/" + arr[i][j].stt + ".jpg"), JPEGSaveOptions, true, Extension.LOWERCASE);
            ...
            for (var ban = 0; ban <= type[arr].length - 1; ban++) { // loop  1 bàn
                ...
                for (var i = 0; i <= type[arr][ban].length - 1; i++) { // loop 1 hàng
                    for (var j = 0; j <= type[arr][ban][i].length - 1; j++) { // lop 1 cái
                        ...
                        try {
                            app.open(File("~/Desktop/file design/" + type[arr][ban][i][j].idDesign + ".tif"));
                        } catch (error) {
                            app.documents.add(1200, 2400, 300, "aaaa");

                        }

                        var checkIdDesign = type[arr][ban][i][j].idDesign.split("-");
                        ...
                        if (checkIdDesign != "spot") {
                            if (app.activeDocument.artLayers.length === 1) { // nhân đôi layer, kiểm tra nếu có 2 layer là phải làm lại từ đầu
                                ...
                                { // xử lý và đưa ảnh sang bàn silicon
                                    ...
                                    if (type[arr][ban][i][j].name.slice(0, 1) == "i")
...
function createTableLed(data) {
    ...
    for (var i = 0; i <= arr.length - 1; i++) { // loop làm file in
        ...
        { // lưu file khung
            ...
            if (!folder1.exists) { folder1.create(); }
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + (i + 1) + " ngay " + day + "/khung.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
        ...
        {// lưu file in 1
            ...
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + (i + 1) + " ngay " + day + "/in 1.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
        ...
        { // lưu file in 2
            ...
            showRGBChannel();
            app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + (i + 1) + " ngay " + day + "/in 2.tif"), TiffSaveOptions, false, Extension.LOWERCASE);
    ...
    { // tạo nhãn
        ...
        for (var i = 0; i <= arr.length - 1; i++) {
            for (var j = 0; j <= arr[i].length - 1; j++) {
                moveTem(arr[i][j], day, mounth);
                var folder1 = Folder("~/Desktop/in an/Led " + (i + 1) + " ngay " + day + "/tem");
                if (!folder1.exists) { folder1.create(); }
                app.activeDocument.saveAs(Folder("~/Desktop/in an/Led " + (i + 1) + " ngay " + day + "/tem/" + arr[i][j].stt + ".jpg"), JPEGSaveOptions, true, Extension.LOWERCASE);
...
function moveTem(item, day, mounth) {
    ...
    app.activeDocument.artLayers.getByName("date").textItem.contents = item.name + "     " + day + "-" + mounth;
    app.activeDocument.artLayers.getByName("idClient").textItem.contents = item.idClient;
...
function LEDOneTable(arr, wAll, hAll, yPosition, xPosition, hLast, wLast, lastName, stt, i) {
    ...
    for (var j = 0; j <= arr[i].length - 1; j++) {
        ...
        // lam SPOT2
        try {
            app.open(File("~/Desktop/file design/" + arr[i][j].idDesign + "flash.tif"));
        } catch (error) {
            app.documents.add(1200, 2400, 300, "aaaa");
        }
        if (app.activeDocument.width !== 1200 || app.activeDocument.height !== 2400) {
